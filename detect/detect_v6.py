...
from Judge2Rooms import judge, statistic, __init
...
weights_path = "/home/ucar/yolov5-v1.0/weights/new.pt"
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if __count <= picPerRoom:
                        statistic(statistic_vector=statisticVec1, label=names[int(cls)], val=conf, xywh=xywh)
                        print(names[int(cls)] + "in Room1    " + "p=%.4f" % conf)
                    elif __count > picPerRoom and __count < picPerRoom * 2:
                        statistic(statistic_vector=statisticVec2, label=names[int(cls)], val=conf, xywh=xywh)
...
def yolo_callback(msg):
    time1 = time.time()
    __init(statisticVec1, statisticVec2, msg)
    # 分别进行识别
    detect(source=img_path, weights=weights_path, conf_thre=0.5, save_img=False)

    # 进行判断
    ans = judge(statisticVec1, statisticVec2)
...
def bo_bao(room_flag):
    if room_flag == 'room_B':
        if ans['B'] == 'kitchen':
            playsound("./B_kitchen.wav")
        elif ans['B'] == 'parlour':
            playsound("./B_parlour.wav")
        elif ans['B'] == 'bedroom':
            playsound("./B_bedroom.wav")

    elif room_flag == 'room_C':
        if ans['C'] == 'kitchen':
            playsound("./C_kitchen.wav")
        elif ans['C'] == 'parlour':
            playsound("./C_parlour.wav")
        elif ans['C'] == 'bedroom':
            playsound("./C_bedroom.wav")
        
    elif room_flag == 'room_D':
        if ans['D'] == 'kitchen':
            playsound("./D_kitchen.wav")
        elif ans['D'] == 'parlour':
            playsound("./D_parlour.wav")
        elif ans['D'] == 'bedroom':
            playsound("./D_bedroom.wav")
    else:
