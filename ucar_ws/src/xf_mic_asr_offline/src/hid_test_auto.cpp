...
std::string awake_words_topic = "/mic/awake/words";
		...
		if (result.whether_recognised)
		{
			if (result.whole_content) //确保有反馈
			{
				...
				if (effective_ans.effective_confidence >= req.confidence_threshold) //如果大于置信度阈值则进行显示或者其他控制操作
				{
					...
					res.fail_reason = "";
                                        std_msgs::String b;
                                        b.data=effective_ans.effective_word;
                                        pub_awake_words.publish(b);
...
int main(int argc, char *argv[])
{
	...
	major_mic_pub = ndHandle.advertise<std_msgs::Int8>(major_mic_topic, true, 1);
        /*　topic 发布启动词*/
	pub_awake_words = ndHandle.advertise<std_msgs::String>(awake_words_topic, true, 1);
