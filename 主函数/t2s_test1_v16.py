...
def Navigation_Main():
    ...
    target_C = Pose(Point(3.250, -4.500, 0.000),
    ...
    # target_D_out = Pose(Point(1.800, -2.800, 0.000),
    #               Quaternion(0.000, 0.000, 1.000, 0.000))
    ...
    if wake_key != 'Jesus':            # a != '370'
        ...
        pub_room = rospy.Publisher('enter_room', Int32, queue_size = 1)
        ...
        if enter_three_rooms == True:
            ...
            if room_flag == 0:
                num = Goto_Target(num, target_B, move_base, pub_cmd, pub_photo, position_B)
                num = Goto_Target(num, target_C, move_base, pub_cmd, pub_photo, position_C)
                Set_Position(move_base, target_D_out)
                while True:
            ...
            # 进入B,D房间识别
                ...
                num = Goto_Target(num, target_B, move_base, pub_cmd, pub_photo, position_B)
                Set_Position(move_base, target_C_out)
                while True:
                    state = move_base.get_state()
                    if state == GoalStatus.SUCCEEDED:
                        break
            ...
            # 进入C,D房间识别
                ...
                Set_Position(move_base, target_B_out)
                while True:
                    state = move_base.get_state()
                    if state == GoalStatus.SUCCEEDED:
                        break
