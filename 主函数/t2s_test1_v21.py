...
# 说明：
# 1.D区抢占目标点只有在只进两个房间且进B，C两个房间识别时生效
# 2.出弯抢占目标点均生效
...
def Navigation_Main():
    ...
    Delete_Photos()
    # 设置目标点坐标和朝向
    target_bendout = Pose(Point(5.500, -1.500, 0.000),
    ...
    target_C = Pose(Point(3.050, -4.500, 0.000),
                  Quaternion(0.000, 0.000, -0.707, 0.707))
    ...
    target_D_out = Pose(Point(1.250, -5.500, 0.000),
                  Quaternion(0.000, 0.000, -0.707, 0.707))
    target_E = Pose(Point(0.100, -1.080, 0.000),
        ...
        if enter_three_rooms == True:
            ...
            if room_flag == 0:
                ...
                while True:
                    ...
                    if pos_X_aver < 2.50 and pos_Y_aver < -2.50:
                        break
                Clear_Costmap()
...
def Goto_Target(num, target, move_base, pub_cmd, pub_photo, position):
    ...
    if rot_flag == 0:
        ...
        num = Rotate_Ceaselessly_Photo(num, pub_cmd, pub_photo, 1.5, 0.30, position)
    else:
