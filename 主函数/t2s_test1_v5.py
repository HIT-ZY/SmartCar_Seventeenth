...
def Navigation_Main():
    ...
    if wake_key != 'Jesus':            # a != '370'
        ...
        rospy.loginfo("OK")
        # B, C, D三个房间
        num = Goto_Target(num, target_B, move_base, pub_cmd)
        num = Goto_Target(num, target_C, move_base, pub_cmd)
        num = Goto_Target(num, target_D, move_base, pub_cmd)
        # 终点E
...
def Goto_Target(num, target, move_base, pub):
    ...
    while True:
        ...
        if state == GoalStatus.SUCCEEDED:
            break
    # 此时已经到达目标点
    num = Rotate_Ceaselessly(num, pub, 1.0, 6.0)
    # Quaternion_test(num)
...
def Rotate_Ceaselessly(num, pub, az, during_time):
    Take_Photo(num)
    ...
    while True:
        ...
        running_time = running_time.secs
        if running_time > rospy.Duration(during_time):
            break
    num += 6
    return num
