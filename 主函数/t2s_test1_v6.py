...
def Navigation_Main():
    ...
    if wake_key != 'Jesus':            # a != '370'
        ...
        rospy.loginfo("OK")
        # B, C, D三个房间
        num = Goto_Target(num, target_B, move_base, pub_cmd, pub_photo)
        num = Goto_Target(num, target_C, move_base, pub_cmd, pub_photo)
        num = Goto_Target(num, target_D, move_base, pub_cmd, pub_photo)
        # 终点E
...
# 行至目标点拍照
def Goto_Target(num, target, move_base, pub_cmd, pub_photo):
    ...
    while True:
        ...
        if state == GoalStatus.SUCCEEDED:
            break
    # 此时已经到达目标点
    num = Rotate_Ceaselessly(num, pub_cmd, pub_photo, 1.0, 0.7)
    # Quaternion_test(num)
...
def Rotate_Ceaselessly(num, pub_cmd, pub_photo, az, every_time):
    '''Take_Photo(num)
